...
if submit !=None:
    ...
    q = """ INSERT INTO booking_details(Empname,Empemail, Empdesignation, Empcity, Empstreet, Empexperience, Username,Useremail,Userstreet, Usercity,status,amount,	customer_type) VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',"New",'%s','%s')""" %(empname,empemail,empdeignation,empcity,empstreet,empexperience,username,useremail,userstreet,usercity,empamount,cstype)
    cur.execute(q)
    con.commit()
    print(""""
    <script>
    alert("Booked successfully")
    location.href="user_workers.py?id=%s"
    </script>
    """ %(userid))
